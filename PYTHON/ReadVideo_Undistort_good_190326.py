...
my_videofile = 'P5_expo200ms_Mac.tif'

#my_videofile = 'Haselpollen-groß-1-Huawei.mp4.tif';
#my_videofile = '2018-11-15 14.33.08.mp4.tif'

scalesize = 1.04 # scaling factor to get rid of the MP4 compression artifact?!
...
psfradius = 1.5             # number of pixels forming the gaussian PSF
mypinholesize = 7           # pixels of raw-frame holding the psf (even number only!)
...
# Super=Max + Min-2*Avg
superconfocal = np.max(ismstack, axis=0)+np.min(ismstack,axis=0)-2*scipy.ndimage.filters.gaussian_filter(np.mean(ismstack,axis=(0)), 0)
plt.figure()
